...
class Action {
    constructor() {
        ...
        this.organisation = process.env.INPUT_ORGANISATION || process.env.ORGANISATION
    ...
    _pushPackage(version, name) {
        ...
        const nugetPushUrl = `https://${this.nugetSource}/${this.organisation}/index.json`
        console.log(`NuGet Source: ${nugetPushUrl}`)
        ...
        const pushCmd = `dotnet nuget push *.nupkg -s ${nugetPushUrl} -k ${this.nugetKey} --skip-duplicate ${!this.includeSymbols ? "-n 1" : ""}`,
    ...
    _checkForUpdate() {
        ...
        let options = {
            protocol: 'https:',
            host: this.nugetSource,
            path: `/${this.organisation}/${this.packageName}/index.json`,
            headers: {
